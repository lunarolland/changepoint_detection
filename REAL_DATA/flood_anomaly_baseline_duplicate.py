...
# =========================================================
# This creates a VIRTUAL timeline in the focus window:
# Sep 12 "uses pixels" from Sep 28, etc.
# Outside focus, everything stays normal.
...
def build_focus_virtual_dates() -> List[date]:
    """The dates you want to appear in the focus window timeline."""
    # If you toggle DUPLICATE_FOCUS_DAILY off, we won't use this.
...
def main():
    ...
    with rasterio.open(base_tif_path) as base_src, rasterio.open(extra_tif_path) as extra_src:
        ...
        # Inside focus:
        # Option A (normal): use actual EXTRA acquisition dates in focus window.
        # Option B (your request): use VIRTUAL daily dates (with duplication map), but read pixels from mapped source dates.
        if DUPLICATE_FOCUS_DAILY:
            ...
            # Ensure the needed SOURCE dates exist in EXTRA
            ...
            focus_dates_window = focus_virtual_dates  # these are the dates you will OUTPUT/plot
        else:
            focus_dates_window = [
        ...
        # Final timeline: BASE outside focus + (EXTRA-derived) inside focus
        ...
        if SHOW_PLOTS:
            ...
            for d_virtual in dates_window:
                if in_focus_window(d_virtual):
                    # focus: EXTRA (possibly duplicated)
        ...
        for d_virtual in dates_window:
            ...
            print(f"{d_virtual} [{src_tag}] (pixels={d_source}) flooded_fraction={frac*100:.3f}%")
        ...
        # =====================================================
        # TIME SERIES PLOT (Flood Intensity Over Time)
        # =====================================================
